function integrate_bb( b, w, P, nstep; fplot=false, fsave=false, nsave=10)
   ...
   # we assume h to be always 0
	...
	P.p -= 0.0001
  	...
  	# add noise once in a while, "to reduce numerical artifacts"
  	  if(mod(i, 2)==0)

@.		u[:,:,1] = u[:,:,1] + 0.0005*rand(P.nx, P.ny)
